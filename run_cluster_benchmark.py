...
def process_worker(args):
    try:
        ...
        # Precision spin-wait for Wave 1
        if target_time_w1 > 0:
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            try:
                ...
                for f in futures1:
                    try:
                        success, lat = f.result(timeout=60.0)
                        wave1_results.append((success, lat))
                    except Exception:
                        wave1_results.append((False, 60000.0))
            finally:
                with w1_done_lock:
                    w1_done_counter.value += 1

            w2_ready_event.wait(timeout=60.0)
            ...
            for f in futures2:
                try:
                    ...
                    wave2_results.append((False, 60000.0))

        return {"wave1": wave1_results, "wave2": wave2_results}
    except Exception:
        with w1_done_lock:
            w1_done_counter.value += 1
        return {"wave1": [(False, 0.0)] * num_threads, "wave2": [(False, 0.0)] * num_threads}
...
def main():
    ...
    def coordinate_wave2_barrier():
        ...
        t_w1_wait = time.time()
        while time.time() - t_w1_wait < 60.0:
            with w1_done_lock:
                if w1_done_counter.value >= num_procs:
                    break
            time.sleep(0.01)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="GCSFS_ALTS_CONCURRENCY limit per node (default: 8, 0 to disable)",
    )
    parser.add_argument(
        "--create-mrd-concurrency",
        type=int,
        default=0,
        help="GCSFS_CREATE_MRD_CONCURRENCY limit per node (default: 0, 0 to disable)",
    )
    parser.add_argument(
        "--auth-concurrency",
        type=int,
        default=0,
        help="GCSFS_AUTH_CONCURRENCY limit per node (default: 0, 0 to disable)",
    )
    parser.add_argument(
        "--storage-layout-lock",
        type=str,
        default="false",
        help="GCSFS_STORAGE_LAYOUT_LOCK (default: false)",
    )
    parser.add_argument(
        "--alts-warmup",
        type=str,
        default="false",
        help="GCSFS_ENABLE_ALTS_WARMUP (default: false)",
    )
    parser.add_argument(
        "--directpath-opt",
        type=str,
        default="false",
        help="GCSFS_DIRECTPATH_OPT (default: false)",
    )
                    ...
                    "containers": [
                        {
                            ...
                            "env": [
                                ...
                                {
                                    "name": "GCSFS_AUTH_CONCURRENCY",
                                    "value": str(args.auth_concurrency),
                                },
                                {
                                    "name": "GCSFS_STORAGE_LAYOUT_LOCK",
                                    "value": str(args.storage_layout_lock),
                                },
                                {
                                    "name": "GCSFS_ENABLE_ALTS_WARMUP",
                                    "value": str(args.alts_warmup),
                                },
                                {
                                    "name": "GCSFS_DIRECTPATH_OPT",
                                    "value": str(args.directpath_opt),
                                },
                                {
                                    "name": "NODE_NAME",
                                    "valueFrom": {
    ...
    all_create_mrd_lats = []
    all_create_mrd_lock_lats = []
    all_alts_lats = []
    all_alts_lock_lats = []
    all_layout_rpc_lats = []
    all_layout_lock_lats = []
    all_layout_total_lats = []

    for pod in sorted(pod_names):
        pod_log, _, _ = run_cmd(f"kubectl logs {pod} --tail=20000", check=False)
        import re

        # Parse create_mrd
        for m in re.finditer(r"init_mrd for [^\n]+ completed in [0-9.]+ ms \([^\n]*?wait_lock:\s*([0-9.]+)\s*ms,\s*exec_mrd:\s*([0-9.]+)\s*ms", pod_log):
            all_create_mrd_lock_lats.append(float(m.group(1)))
            all_create_mrd_lats.append(float(m.group(2)))

        # Fallback create_mrd matches
        if not all_create_mrd_lats:
            mrd_matches = re.findall(r"(?:exec_mrd|create_mrd):\s*([0-9.]+)\s*ms", pod_log)
            for m in mrd_matches:
                all_create_mrd_lats.append(float(m))

        # Parse ALTS warmup
        for m in re.finditer(r"ALTS channel warmup for [^\n]+ completed in [0-9.]+ ms \([^\n]*?wait_lock:\s*([0-9.]+)\s*ms,\s*exec_ready:\s*([0-9.]+)\s*ms\)", pod_log):
            all_alts_lock_lats.append(float(m.group(1)))
            all_alts_lats.append(float(m.group(2)))

        # Fallback ALTS matches
        if not all_alts_lats:
            alts_matches = re.findall(r"exec_ready:\s*([0-9.]+)\s*ms", pod_log)
            for a in alts_matches:
                all_alts_lats.append(float(a))

        # Parse Storage Layout Lookups
        for m in re.finditer(r"storage_layout lookup for [^\n]+ completed in\s*([0-9.]+)\s*ms\s*\(wait_lock:\s*([0-9.]+)\s*ms,\s*exec_rpc:\s*([0-9.]+)\s*ms", pod_log):
            all_layout_total_lats.append(float(m.group(1)))
            all_layout_lock_lats.append(float(m.group(2)))
            rpc_val = float(m.group(3))
            if rpc_val > 0.0:  # Only count actual RPC executions (not lock wait cache hits)
                all_layout_rpc_lats.append(rpc_val)
    ...
    if not args.scalar:
        ...
        if all_alts_lock_lats:
            print_table(
                "ALTS Rate-Limiting Slot Lock Wait Time",
                all_alts_lock_lats,
                num_nodes,
                num_ranks,
                num_workers,
        ...
        if all_create_mrd_lock_lats:
            print_table(
                "create_mrd Rate-Limiting Slot Lock Wait Time",
                all_create_mrd_lock_lats,
                num_nodes,
                num_ranks,
                num_workers,
                num_procs,
                args.threads,
                len(all_create_mrd_lock_lats),
                len(all_create_mrd_lock_lats),
                0,
                0,
            )

        if all_layout_rpc_lats:
            print_table(
                "Storage Layout get_storage_layout Pure RPC Execution Time",
                all_layout_rpc_lats,
                num_nodes,
                num_ranks,
                num_workers,
                num_procs,
                args.threads,
                len(all_layout_rpc_lats),
                len(all_layout_rpc_lats),
                0,
                0,
            )

        if all_layout_lock_lats:
            print_table(
                "Storage Layout Intra-Process Lock Wait Time",
                all_layout_lock_lats,
                num_nodes,
                num_ranks,
                num_workers,
                num_procs,
                args.threads,
                len(all_layout_lock_lats),
                len(all_layout_lock_lats),
                0,
                0,
            )

        if all_layout_total_lats:
            print_table(
                "Storage Layout Lookup Total Phase Latency",
                all_layout_total_lats,
                num_nodes,
                num_ranks,
                num_workers,
